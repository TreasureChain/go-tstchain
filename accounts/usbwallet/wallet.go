...
type wallet struct {
	...
	deriveChain    tstchain.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain tstchain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
