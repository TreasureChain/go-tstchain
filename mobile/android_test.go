...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.tstchain", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for go-tstchain issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/world/baolian/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "world.baolian", "github.com/TreasureChain/go-tstchain/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "tstchain", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="world.baolian.gethtest"
