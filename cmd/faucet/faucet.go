...
import (
	...
	"github.com/TreasureChain/go-tstchain/ethclient"
...
var (
	...
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Tsts to pay out per user request")
...
var (
	tst = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Tsts", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type faucet struct {
	...
	client *ethclient.Client   // Client connection to the Tstchain chain
...
// apiHandler handles requests for Tst grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/TreasureChain/go-tstchain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), tst)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, tst)
