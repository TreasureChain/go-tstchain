...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	...
	echo $'gtst --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ethstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppethNetworkID/{{.NetworkID}}/g' dist/js/tstwallet-master.js && \
	sed -i 's/PuppethNetwork/{{.Network}}/g'     dist/js/tstwallet-master.js && \
	sed -i 's/PuppethDenom/{{.Denom}}/g'         dist/js/tstwallet-master.js && \
	sed -i 's/PuppethHost/{{.Host}}/g'           dist/js/tstwallet-master.js && \
	sed -i 's/PuppethRPCPort/{{.RPCPort}}/g'     dist/js/tstwallet-master.js
