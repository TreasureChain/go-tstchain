...
func (w *wizard) deployNode(boot bool) {
	...
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an tstbase to mine against
			fmt.Println()
			if infos.tstbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.tstbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.tstbase)
				infos.tstbase = w.readDefaultAddress(common.HexToAddress(infos.tstbase)).Hex()
			}
